...
class DataGenerator(keras.utils.Sequence):
    ...
    def __data_generation(self, list_IDs_temp):
        ...
        # Initialization
        X = np.zeros(
            (self.params.batchSize, self.params.imageSize, self.params.imageSize, self.params.imageDim), dtype=float)
        
        y = np.zeros(
            (self.params.batchSize, self.params.nClass), dtype=int)
...
class getData():
    ...
    def getList(self):
        ...
        for root, dirs, files in os.walk(self.pathImage):
            for file in files:
